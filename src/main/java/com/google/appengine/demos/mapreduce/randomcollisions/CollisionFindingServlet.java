...
public class CollisionFindingServlet extends HttpServlet {
  ...
  private void writeResponse(HttpServletResponse resp) throws IOException {
    ...
    try (PrintWriter pw = new PrintWriter(resp.getOutputStream())) {
      pw.println("<html><body>" + "<form method='post' > <input type='hidden' name='token' value='"
          ...
          + "<input name='gcs_bucket' value='ozarov-javamrsample1'/> (Leave empty to use the app's default bucket) <br />"
          + "Backend: <input name='backend' /><br />"
          + "Module: <input name='module' /><br />"
          + "Queue: <input name='queue' value='mapreduce-workers'/><br />"
          + "Starting integer to test: <input value='0' name='start' /> <br />"
          + "Final integer to test: <input value='300' name='limit' />"
          + "(Must be greater than starting integer and less than 2^31-1) <br />"
          + "Number of shards <input value='1' name='shards' />"
          + "(Must be between 1 and 100) <br /> Config <select name='config'>\n" +
              "  <option value=\"ONE\">App without modules using a backend.</option>\n" +
              "  <option value=\"TWO\">App with modules using a backend.</option>\n" +
              "  <option value=\"THREE\">3</option>\n" +
              "  <option value=\"FOUR\">4</option>\n" +
              "</select><br /><input type='submit' value='Start MapReduce' />"
          + "</div> </form> </body></html>");
    }
  }
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse resp) throws IOException {
    ...
    String queue = getStringParam(req, "queue", null); // "mapreduce-workers");
    String module = getStringParam(req, "module", null); //"mapreduce");
    ...
    Config config = Config.valueOf(req.getParameter("config"));
    ...
    MapReduceSettings settings = getSettings(
        bucket,
        getStringParam(req, "backend"),
        getStringParam(req, "module"),
        getStringParam(req, "queue"));
    String id = MapReduceJob.start(mapReduceSpec, settings);
    // [END start_mapreduce]
    resp.sendRedirect("/_ah/pipeline/status.html?root=" + id);
  }

  static String getStringParam(HttpServletRequest req, String param) {
  ...
  static String getBucketParam(HttpServletRequest req) {
    String bucket = getStringParam(req, "gcs_bucket");
    if (bucket == null) {
  ...
  public static MapReduceSettings getSettings(
      String bucket, String backend, String queue, String module) {
